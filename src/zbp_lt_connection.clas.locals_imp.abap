...
CLASS lhc_connection IMPLEMENTATION.
  ...
  METHOD CheckCarrierID.
    READ ENTITIES OF zlt_r_connection
    IN LOCAL MODE ENTITY Connection FIELDS ( CarrierID )
    WITH CORRESPONDING #( keys )
    RESULT DATA(connections).
  ...
  METHOD GetCities.
    READ ENTITIES OF zlt_r_connection
    IN LOCAL MODE ENTITY Connection
    ...
    LOOP AT connections INTO DATA(connection).
      ...
      DATA connections_upd TYPE TABLE FOR UPDATE zlt_r_connection.
      connections_upd = CORRESPONDING #( connections ).

      MODIFY ENTITIES OF zlt_r_connection
      IN LOCAL MODE ENTITY Connection
